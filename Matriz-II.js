...
function deslizAutomatic(){
  var elementosExcluidos = ['buscador','container01','links-inicialesI', 'links-iniciales', 'conteneMantaut', 'conti-boton','troubleshooting','canvascontainer2','MiGrafica','canvascontainer3','kaizenCont','vidElem'];
  for (var i = 0; i < allContenedores.length; i++) {
    var elemento = document.getElementById(allContenedores[i]);
    if (elemento) {
      elemento.style.display = elementosExcluidos.includes(allContenedores[i]) ? 'flex' : 'none';
...
const applyFilters = () => {
  ...
  const applyFiltersII = () => {
    videoPrisma.style.transform = `scale(${zoomAmount}%) rotate(${degreesAmount}deg)`; // Conserva el zoom actual y aplica solo la rotación
  };
...
function interfazAvance() {
  ...
  aplique.style.visibility = 'visible'  
...
function Geometria() {
  var contiBoton = document.getElementById('container_interfaz')
